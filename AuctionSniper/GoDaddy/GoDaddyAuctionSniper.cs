...
namespace GoDaddy
{
    public class GoDaddyAuctionSniper : HttpBase
    {
        ...
        {
            return html.Contains("shopperId:") && !html.Contains("shopperId: '{{pc:shopperid}}'");
        }
        ...
        public bool Login(int attempNo = 0)
        {
            ...
            if (QuerySelector(hdoc.DocumentNode, "img[class='LBD_CaptchaImage']") != null)
            {
                ...
                try
                {
                    ...
                    var user = SessionDetails.DeathByCapture.Username; //ds.SystemConfig.First(x => x.PropertyID == "DBCUser").Value;
                    var pass = SessionDetails.DeathByCapture.Password; //ds.SystemConfig.First(x => x.PropertyID == "DBCPass").Value;
